...
def create_task_category(db: Session, task_category: schema.TaskCategoryCreate):
...
def get_task_category(db: Session, task_category_id: int):
...
def update_task_category(db: Session, task_category: schema.TaskCategoryBase, task_category_id: int):
...
def delete_task_category(db, task_category_id):
    db_task_category = db.query(model.TaskCategory).filter(model.TaskCategory.id == task_category_id).first()
